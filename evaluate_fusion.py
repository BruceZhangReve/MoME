...
from llm.utils import load_llm_for_evaluation
from utils import (
...
def evaluate_full_test_set(
    ...
    if args.task in ['energy_forecast', 'healthus_forecast', 'healthafr_forecast', 'socialgood_forecast', 'epa_air_forecast', 'clustertrace_forecast', 'gdelt_aus_forecast', 'gdelt_can_forecast', 'ilinet_forecast', 'mm_weather_forecast']:
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            pred_seq = downstream_head(final_hidden) # [B, out_len * C] or [B, out_len]
            ...
            if len(true_seq.shape) == 3:
                ...
                pred_seq = pred_seq.reshape(B, C, out_len)
            # Compute metric on normalized space for these multi-channel datasets
...
def main():
    ...
    if args.eval_mode == "random_sample":
        ...
        if args.task in ["weather_forecast", "energy_forecast", "finance_forecast", 'healthus_forecast', 'healthafr_forecast', 'socialgood_forecast', 'epa_air_forecast', 'clustertrace_forecast', 'gdelt_aus_forecast', 'gdelt_can_forecast', 'ilinet_forecast', 'mm_weather_forecast']:
