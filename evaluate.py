...
def get_ts_embed(
    ...
    """
    Return ts_embed and (optionally) selected_experts for visualization.

    Returns:
        if return_expert_selection=True: (ts_embed, selected_experts)
        else: ts_embed
    """
    ...
    if args.ts_encoder in ["MoMe", "mmlinear", "MiTransformer"]:
    ...
    if args.ts_encoder == 'iTransformer':
        ...
        if args.ts_encoder == 'MoMe':
            # Unified MoMe returns selected_experts if return_expert_selection=True
            if ts_encoder.return_expert_selection:
                ts_embed, selected_experts = ts_encoder(ts, Ins_tk)
                return ts_embed, selected_experts
            else:
                ts_embed = ts_encoder(ts, Ins_tk)
                return ts_embed
        else:
            # Original separate models
            if ts_encoder.modulation == False:
                ts_embed = ts_encoder(ts)  # [B, 1, in_len] -> [B, 1*P, d]
            else:
                ts_embed = ts_encoder(ts, Ins_tk)  # [B, 1, in_len] -> [B, 1*P, d]
...
def load_model_components(args):
    ...
    if args.ts_encoder == 'iTransformer':
        ...
        ts_token_num = args.in_len
        lambda_e = getattr(args, 'lambda_e', 0.75)
        ts_encoder = MiTransformer(
            n_vars=args.n_vars, seq_len=ts_token_num, d_model=args.hidden_dim, dropout=0.1,
            topk=args.topk, router_modulation=args.router_modulation, n_experts=args.n_experts,
            n_heads=2, n_layers=args.n_layers, modulation=args.modulation, lambda_e=lambda_e
        ).to(args.device)
        ...
        ts_encoder = MMLinear(in_len=args.in_len, out_len=args.out_len,
                              ...
                              router_modulation=args.router_modulation,
                              lambda_e=lambda_e).to(args.device)
        ...
        # Unified MoMe model with all features
        # router_modulation: enable router modulation
        # return_expert_selection: return selected_experts for visualization/case study
        router_modulation = getattr(args, 'router_modulation', args.router_modulation)
        return_expert_selection = getattr(args, 'return_expert_selection', False)
        lambda_e = getattr(args, 'lambda_e', 0.75)
        ...
        ts_encoder = MoMe(
            ...
            modulation=args.modulation,
            router_modulation=router_modulation,
            lambda_e=lambda_e,
            return_expert_selection=return_expert_selection
        ).to(args.device)
        ts_token_num = ts_encoder.patch_num
        for param in ts_encoder.parameters():
            param.requires_grad = False
...
def calculate_metrics(pred_seq, true_seq, metric_names, task):
    ...
    if task == 'finance_forecast' or task == 'environment_forecast':
...
def evaluate_test_sample(
    ...
    """
    Evaluate model on a single random test sample for visualization.
    ...
    embed_output = get_ts_embed(args, ts_encoder, random_batch, llm, language_instructor)

    if isinstance(embed_output, tuple):
        inputs_embeds, selected_experts = embed_output
    else:
        inputs_embeds = embed_output
        selected_experts = None

    # Get denormalization parameters
    if 'denorm_params' in random_batch:
    ...
    # Denormalize predictions if available
...
def evaluate_full_test_set(
    ...
    if args.task == 'weather_forecast':
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            embed_output = get_ts_embed(args, ts_encoder, batch, llm, language_instructor)
            if isinstance(embed_output, tuple):
                inputs_embeds, _ = embed_output
            else:
                inputs_embeds = embed_output
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            if selected_experts is not None:
                if 'selected_experts' not in results:
                    results['selected_experts'] = []
        ...
        raise KeyError("args.task not supported")


@torch.no_grad()
def collect_expert_selection(
    args,
    llm,
    ts_encoder,
    downstream_head,
    ...
    """
    Collect input sequences and selected experts for case study/visualization.
    This replaces the original evaluate.py functionality.
    """
    ts_encoder.eval()
    if downstream_head is not None:
        downstream_head.eval()
    ...
    pbar = tqdm(test_loader, desc="Collecting Expert Selection Data")
    ...
    for batch_idx, batch in enumerate(pbar):
        ...
        if isinstance(embed_output, tuple):
            _, selected_experts = embed_output
        else:
            selected_experts = None
...
def parse_args():
    ...
    parser.add_argument("--modulation", action="store_true", help="Enable language modulation")
    parser.add_argument("--patch_len", type=int, default=4)
    parser.add_argument("--ts_encoder", type=str, default="MoMe")
    ...
    parser.add_argument("--moe_mode", default="No")
    ...
    parser.add_argument("--router_modulation", action="store_true", help="Enable router modulation")
    parser.add_argument("--return_expert_selection", action="store_true", help="Return selected experts for visualization (MoMe)")
    parser.add_argument("--lambda_e", type=float, default=0.75)

    parser.add_argument("--stride", type=int, default=4)

    # -------- Numerical Safety & Dtypes --------
    parser.add_argument("--pre_align_clip", type=float, default=1e5,
                        help="If > 0, clamp TS embedding to [-clip, clip] BEFORE AlignLayer.")
    parser.add_argument("--use_bfloat16", action="store_true",
                        help="Load LLM in bfloat16 if supported.")
    ...
    parser.add_argument("--eval_mode", type=str, default="full_test",
                        help="full_test: evaluate full test set; random_sample: visualize single sample; expert_selection: collect expert selection")
    parser.add_argument("--save_full_predictions", action="store_true", help="Save full predictions")
    parser.add_argument("--use_checkpoint_args", action="store_true", help="Use config from checkpoint")
...
def main():
    ...
    if args.eval_mode == "expert_selection":
        # Collect expert selection for case study (original evaluate.py functionality)
        if not args.return_expert_selection:
            print("WARNING: return_expert_selection should be True for expert_selection eval_mode!")
            print("Setting return_expert_selection = True automatically...")
            args.return_expert_selection = True
            if hasattr(ts_encoder, 'return_expert_selection'):
                ts_encoder.return_expert_selection = True

        results = collect_expert_selection(args, llm, ts_encoder, downstream_head, Instructor, test_loader)
        pred_path = os.path.join(args.output_dir, "test_results.npz")
        np.savez(
            pred_path,
            input_seq=results["input_seq"],
            selected_experts=results.get("selected_experts", []),
        )
        print(f"Expert selection results saved at: {pred_path}")

    elif args.eval_mode == "random_sample":
        # Single sample evaluation with visualization
        ...
        result_summary = {
            ...
            "total_samples": len(results["predictions"]) if "predictions" in results else len(results["input_seq"]),
            "task": args.task,
        }
        ...
        print(f"Evaluation results saved to test_set_evaluation_summary.json")

        if args.save_full_predictions:
            pred_path = os.path.join(args.output_dir, "test_predictions.npz")
            if "selected_experts" in results:
                np.savez(
                    pred_path,
            ...
            print(f"Complete results saved at: {pred_path}")
    else:
        raise ValueError(f"Unknown eval_mode: {args.eval_mode}. Choose from: full_test, random_sample, expert_selection")
