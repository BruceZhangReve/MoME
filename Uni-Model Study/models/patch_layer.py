...
class Patch_backbone(nn.Module):
    def __init__(self, args, device,**kwargs):
        ...
        # Backbone 
        self.backbone = TV_Encoder(self.n_vars, patch_num=patch_num, topk=self.topk, if_moe=self.if_moe, patch_len=self.patch_len, max_seq_len=self.max_seq_len, 
        ...
        if self.pretrain_head: 
            ...
            self.head = Flatten_Head(self.individual, self.n_vars, self.head_nf, self.out_len, head_dropout=0.0, n_cluster=self.n_cluster, if_moe=self.if_moe, device=self.device)
    ...
    def forward(self, z, cls_emb, prob):
        ...
        # model
        z, cls_emb, gate_score_lis = self.backbone(z, cls_emb, prob)  
        z = self.head(z, prob)                                      # z: [bs, nvars, d_model, patch_num]
        #if self.if_moe:
            #gate_prob = torch.mean(torch.stack(gate_score_lis, dim=0), dim=0) #[n_vars, n_cluster]
            #z = self.head(z, gate_prob)        
        #else:
        ...
        z = z.permute(0,2,1)
        return z, cls_emb, gate_score_lis
...
class Flatten_Head(nn.Module):
    def __init__(self, individual, n_vars, nf, target_window, head_dropout=0, n_cluster=1, if_moe=False, device=None):
        super().__init__()
        #note: nf = d_model * patch_num
...
class TV_Encoder(nn.Module):
    def __init__(self, c_in, patch_num, topk, if_moe, patch_len, max_seq_len=1024, n_cluster=1, 
        ...
        self.encoder = nn.ModuleList([TimeVarAttentionLayer(c_in, d_model, n_heads=n_heads, d_k=d_k, d_v=d_v, d_ff=d_ff, 
                                                            ...
                                                            dropout=dropout, pre_norm=pre_norm, n_cluster=n_cluster, patch_num=patch_num, topk=topk, if_moe=if_moe) 
    ...
    def forward(self, x, cls_emb, prob) -> Tensor:                                              
        ...
        for mod in self.encoder: 
            if not self.if_moe:
                ...
                z, cls_emb, gate_score = mod(z, cls_emb, prob)  
                gate_score_list.append(gate_score) #track gate score at every layer
...
class TimeVarAttentionLayer(nn.Module):
    def __init__(self, n_vars, d_model, n_heads, d_k=None, d_v=None, d_ff=256, store_attn=False,
                 norm='BatchNorm', attn_dropout=0, dropout=0., bias=True, pre_norm=False, n_cluster=1, patch_num=1, topk=1, if_moe=False):
        super().__init__()
        ...
        if not if_moe:
            ...
            self.ff = nn.Sequential(nn.Linear(2*d_model, d_ff, bias=bias),
                                    nn.GELU(),
                                    nn.Dropout(dropout),
                                    nn.Linear(d_ff, d_model, bias=bias))
        ...
        self.if_moe = if_moe
        if self.if_moe:
            #self.Gating = GatingNetwork(patch_num * d_model, n_cluster, topk)
            self.proj = nn.Linear(patch_num * d_model, d_model) #v3
            self.Gating = GatingNetwork_v3(d_model, n_cluster) #v3
            self.experts_r = nn.ModuleList([self.ff for _ in range(n_cluster)])
    ...
    def forward(self, src, clusters, prob) -> Tensor:
        ...
        if not self.if_moe:
            ## 5. Feed-Forward
            src2 = self.ff(src) #[bs * nvars, patch_num, d_model] channel independent FFN
            ## 6. Add & Norm
            ...
            gate_scores = self.Gating(self.proj(src_flat), clusters)
            expert_r_outputs = torch.stack(
                [expert_r(torch.cat([src, clusters[k].expand_as(src)], dim=-1)) for k, expert_r in enumerate(self.experts_r)], dim=2) #[bs, nvars, n_clusters, patch_num, d_model]
            y_r = torch.einsum('nk,bnkpd->bnpd', gate_scores, expert_r_outputs)  # [bs, nvars, patch_num , d_model]
