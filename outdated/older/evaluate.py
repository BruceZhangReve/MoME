...
from TS_Encoder.mome_v1 import MoMe
...
####################################################
# we directly copy those modules from train.py
...
def evaluate_test_sample(
    ...
    if args.task == "weather_forecast":
        true_seq = random_batch['output_window'].to(embed_device)

        inputs_embeds, attn_mask, txt_attn_mask = fuse_ts_and_text(
            args, model, ts_encoder, align_layer, random_batch, embed_device, model_dtype, mode="test"
        ) 
        ...
        ground_truth = true_seq.cpu().numpy()  # the appended value should be of shape [B, out_len], bu usually B=1
        ...
        metric1 = mse_criterion(pred_seq, true_seq)#calculate_mape(pred_seq, true_seq)
        metric2 = mae_criterion(pred_seq, true_seq)
        metric_names = ("MSE", "MAE")
    ...
    if args.task in ['weather_forecast','finance_forecast', 'finance_trend_prediction', 'weather_trend_prediction']:
...
def evaluate_full_test_set(
    ...
    if args.task == 'weather_forecast':
        ...
        for batch_idx, batch in enumerate(pbar):
            inputs_embeds, attn_mask, txt_attn_mask = fuse_ts_and_text(
                args, model, ts_encoder, align_layer, batch, embed_device, model_dtype, mode="test"
                )
            ...
            #model = get_transformer_backbone(model)
            #print(inputs_embeds.dtype, attn_mask.dtype) # always bf16, int64
            llm_outputs = model(
                inputs_embeds=inputs_embeds,
                attention_mask=attn_mask,
                use_cache=False
...
def parse_args():
    ...
    #parser.add_argument("--lora_modules", default="q_proj,k_proj,v_proj,o_proj,shared_expert_gate") #"q_proj,k_proj,v_proj,o_proj,gate,gate_proj,up_proj,down_proj"
    ...
    parser.add_argument("--hidden_dim", type=int, default=384) #note 384 is the embedding dimensionf for time-moe
...
def main():
    ...
    if args.eval_mode == "random_sample":
        evaluate_test_sample(args, tokenizer, model, ts_encoder, align_layer, downstream_head,
