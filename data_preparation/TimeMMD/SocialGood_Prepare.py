# -*- coding: utf-8 -*-
# Social Good (US Unemployment by Race, BLS) -> English News Synthesis (DeepSeek)
# v1: Hard-aligned prompt + post-generation alignment checks + retries
#
...
#
# Domain: Monthly unemployment series by race (BLS, 1954–present). We write neutral, observed-only
# syntheses with NO numbers and NO forecasts. Sequences are read from the 'OT' column by default.
...
NUM_PATH = "../../data/raw/TimeMMD/SocialGood/SocialGood.csv" 
OUT_DIR  = "../../data/processed/TimeMMD/SocialGood"  
API_KEY  = "..."  
...
# =====================
# Metrics builder (Social Good: Unemployment)
# =====================
def unemp_metrics_from_io(input_seq, out_seq) -> Dict[str, Any]:
    """
...
# =====================
# HARD-CONSTRAINED Prompt (Unemployment domain)
# =====================
def build_unemp_prompt(input_seq, metrics, unit="month") -> str:
    """
    Hard-constrained prompt for unemployment reporting:
    - LOCKED：past / near-term / future 三个方向必须一致
    - 允许的同义词限定，避免自由发挥导致的矛盾
    - 仅叙述已观察到的劳动力市场现象（调查口径、行业招聘/裁员、季节性上岗/离岗、罢工已发生/已解决、
      已生效的政策/公共项目、极端天气或供应链扰动对用工与求职的影响、调查响应率与抽样波动等）
    - 避免数字、百分比、预测；语气中立；一段话
    """
    ...
    rising_syn  = ["rising", "increasing", "moving higher", "upward", "firming"]
    falling_syn = ["falling", "declining", "moving lower", "downward", "easing"]
    steady_syn  = ["steady", "broadly unchanged", "stable", "little changed", "flat"]

    return f"""
You are a labor & economy reporter. Write ONE concise, professional English bulletin (120–160 words)
about OBSERVED U.S. unemployment conditions over the past {n_periods} {unit}s, with attention to racial group differences
as structured in official reporting.
...
4) Do NOT include any numbers (counts, percentages, rates, percentage points) or dates.
5) Keep a neutral, factual tone; write ONE coherent paragraph.
6) When discussing racial groups, remain respectful and neutral; describe reported patterns without attribution or value judgment.

[OBSERVED CONTEXT YOU MAY USE]
- sector hiring/layoffs and shifts in labor demand already reported,
- seasonal onboarding/offboarding, resolved or ongoing labor actions already documented,
- policy changes and public programs already in effect,
- weather disruptions or supply/logistics constraints already observed,
- survey design/response, sampling variability, data revisions already recorded,
- differences across demographic segments as structured in official releases.
...
BAN_PATTERNS_NEWS = [
    ...
    # 数字/百分比/点数
    r"\d+(\.\d+)?\s*%?", r"\bpercent\b", r"\bpercentage\b", r"\bpercentage\s*point(s)?\b",
    r"\brate(s)?\b(?=\s*\d)", r"\bpoint(s)?\b(?=\s*\d)",
]
def sanitize_news_text(text: str) -> str:
    s = text.strip()
    for pat in BAN_PATTERNS_NEWS:
        s = re.sub(pat, "[redacted]", s, flags=re.IGNORECASE)
    return s

# 方向词典用于校验
DIR_SYNONYMS = {
    "rising":  {"rising","increase","increasing","upward","moving higher","higher","firming","uptick","climb","climbing"},
    "falling": {"falling","decline","declining","downward","moving lower","lower","easing","dip","softening","ebb","drop","dropping"},
...
def generate_unemp_with_alignment(prompt: str, metrics: Dict[str, Any],
                                  model="deepseek-chat", temperature=0.2, max_tokens=260,
                                  max_retries: int = 3) -> str:
    """
    先生成 -> 校验是否与锁定方向一致 -> 不一致则附加纠错提示重试，最多 max_retries 次。
    """
    base_system = "You are a careful labor & economy reporter. You only describe observed context and avoid numbers."
    attempt = 0
    ...
    while attempt <= max_retries:
        try:
            ...
            last_text = sanitize_news_text(resp.choices[0].message.content.strip())
...
# =====================
# Column detection (prefer 'OT')
# =====================
DATE_CANDIDATES = ["month", "date", "period", "start_date", "end_date", "year_month"]
...
def _detect_columns(df: pd.DataFrame) -> Tuple[Optional[str], str]:
    ...
    for c in df.columns:
        if c.lower() in DATE_CANDIDATES:
            date_col = c
            break
    # prefer 'OT' as the value column
    if "OT" in df.columns and pd.api.types.is_numeric_dtype(df["OT"]):
        val_col = "OT"
    else:
        numeric_cols = [c for c in df.columns if c != date_col and pd.api.types.is_numeric_dtype(df[c])]
        assert numeric_cols, "No numeric value column found (and 'OT' missing)!"
...
# Batch generator
# =====================
def attach_unemp_news(df: pd.DataFrame, p_consistent: float = 0.8, seed: int = 42) -> pd.DataFrame:
    rng = random.Random(seed)
    out_rows = []
    for _, row in tqdm(df.iterrows(), total=len(df), desc="Generating unemployment news"):
        x, y = row["input_seq"], row["output_seq"]
        print(x, y)
        prof = unemp_metrics_from_io(x, y)
        ...
        prompt = build_unemp_prompt(input_seq=x, metrics=metrics, unit="month")

        news = "(generation failed)"
        for _ in range(3):
            try:
                news = generate_unemp_with_alignment(prompt, metrics, temperature=0.2, max_tokens=260)
                break
            except Exception:
                time.sleep(1.2)
        print(news)
...
def preview_one_sample(df: pd.DataFrame, idx: Optional[int] = None, seed: int = 123) -> str:
    ...
    row = df.iloc[idx]
    prof = unemp_metrics_from_io(row["input_seq"], row["output_seq"])
    print(
        f"[Preview] row={idx} | "
    ...
    prompt = build_unemp_prompt(input_seq=row["input_seq"], metrics=prof, unit="month")
    return generate_unemp_with_alignment(prompt, prof, temperature=0.2, max_tokens=260)
...
def main():
    ...
    # generate texts
    train_with_text = attach_unemp_news(train_df, p_consistent=p_consistent, seed=42)
    test_with_text  = attach_unemp_news(test_df,  p_consistent=p_consistent, seed=43)
