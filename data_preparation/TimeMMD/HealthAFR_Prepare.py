# -*- coding: utf-8 -*-
# Public Health (Africa) -> English News Synthesis (DeepSeek)
# v1: Hard-aligned prompt + post-generation alignment checks + retries
#
# Indicators used in prompt (same as ILI/US pipeline you approved):
...
#
# Domain: WHO Africa public-health weekly indicators (generic respiratory/syndromic activity).
# We keep reporting strictly observational, with NO numbers and NO forecasts.
# The model output is sanitized to remove numeric content and speculative phrasing.
...
NUM_PATH = "../../data/raw/TimeMMD/HealthAFR/HealthAFR.csv" 
OUT_DIR  = "../../data/processed/TimeMMD/HealthAFR"  
API_KEY  = "..." 
...
# =====================
# Metrics builder (Africa health)
# =====================
def africa_metrics_from_io(input_seq, out_seq) -> Dict[str, Any]:
    """
...
# =====================
# HARD-CONSTRAINED Prompt (Africa health domain)
# =====================
def build_africa_prompt(input_seq, metrics, unit="week") -> str:
...
about OBSERVED respiratory/syndromic activity reported across African countries over the past {n_periods} {unit}s.
...
- national surveillance summaries and sentinel site reports,
- regional heterogeneity across West/East/Central/Southern/North Africa,
- seasonality (e.g., rainy/dry periods) and mobility/travel timing,
- vaccination or public guidance already in effect,
- co-circulation mentions (e.g., other respiratory pathogens) if reported,
- reporting cadence, data completeness, and known lags,
- access constraints (supply/logistics, extreme weather, conflict) when noted.
...
def generate_africa_with_alignment(prompt: str, metrics: Dict[str, Any],
                                   model="deepseek-chat", temperature=0.2, max_tokens=260,
                                   max_retries: int = 3) -> str:
    """
    先生成 -> 检测是否与锁定方向一致 -> 不一致则附加纠错提示重试，最多 max_retries 次。
    """
...
# =====================
# Column detection (Africa)
# =====================
DATE_CANDIDATES = ["week", "date", "start_date", "end_date", "period", "epiweek"]
VALUE_PREFER = ["OT"]
...
def _detect_columns(df: pd.DataFrame) -> Tuple[Optional[str], str]:
    ...
    for c in df.columns:
        if c.lower() in DATE_CANDIDATES:
            date_col = c
            break
    # prefer common value names; else pick first numeric col not equal to date_col
    candidates = [c for c in df.columns if c.lower() in VALUE_PREFER]
    val_col = None
    for c in candidates:
    ...
    if val_col is None:
        ...
        assert numeric_cols, "No numeric value column found!"
...
# Batch generator
# =====================
def attach_africa_news(df: pd.DataFrame, p_consistent: float = 0.8, seed: int = 42) -> pd.DataFrame:
    rng = random.Random(seed)
    out_rows = []
    for _, row in tqdm(df.iterrows(), total=len(df), desc="Generating Africa health news"):
        x, y = row["input_seq"], row["output_seq"]
        print(x,y)
        prof = africa_metrics_from_io(x, y)
        ...
        prompt = build_africa_prompt(input_seq=x, metrics=metrics, unit="week")

        news = "(generation failed)"
        for _ in range(3):
            try:
                news = generate_africa_with_alignment(prompt, metrics, temperature=0.2, max_tokens=260)
                break
            except Exception:
                time.sleep(1.2)
        print(news)
...
def preview_one_sample(df: pd.DataFrame, idx: Optional[int] = None, seed: int = 123) -> str:
    ...
    row = df.iloc[idx]
    prof = africa_metrics_from_io(row["input_seq"], row["output_seq"])
    print(
        f"[Preview] row={idx} | "
    ...
    prompt = build_africa_prompt(input_seq=row["input_seq"], metrics=prof, unit="week")
    return generate_africa_with_alignment(prompt, prof, temperature=0.2, max_tokens=260)
...
def main():
    ...
    # generate texts
    train_with_text = attach_africa_news(train_df, p_consistent=p_consistent, seed=42)
    test_with_text  = attach_africa_news(test_df,  p_consistent=p_consistent, seed=43)
