...
class MMWeatherDataset(Dataset):
    ...
    def _load_data(self):
        ...
        for idx, row in tqdm(df.iterrows(), total=len(df), desc="Processing"):
            try:
                ...
                channels = ['temperature', 'relative_humidity', 'visibility', 'precipitation_3_hour', 'u_wind', 'v_wind']
                input_2d = []
                output_2d = []
                for ch in channels:
                ...
                input_window = torch.tensor(input_2d, dtype=torch.float32)  # [C, in_len]
                output_window = torch.tensor(output_2d, dtype=torch.float32)  # [C, out_len]
...
if __name__ == "__main__":
    data_dir = "./processed/finance/long/train"
    tokenizer = AutoTokenizer.from_pretrained("../llm/Qwen-0.5B")  
