...
class MiTransformer(nn.Module):
    def __init__(self, n_vars, seq_len, d_model, dropout, topk, router_modulation,
                 n_experts, n_heads, n_layers, modulation=True, lambda_e=0.75):
        super(MiTransformer, self).__init__()
        self.n_vars = n_vars
        self.modulation = modulation
        self.seq_len = seq_len
        self.d_model = d_model
        self.encoder = MiTransformer_backbone(
            n_vars, seq_len, d_model, dropout, topk, router_modulation, n_experts,
...
class MiTransformer_backbone(nn.Module):
    def __init__(self, n_vars, in_len, d_model, dropout, topk, router_modulation,
                 n_experts, n_heads, n_layers, modulation=True, lambda_e=0.75):
        ...
        self.seq_len = in_len
        self.output_attention = False      # Whether or not return output attention
        self.use_norm = True               # Use RevIN norm
        self.modulation = modulation
    ...
    def forecast(self, x_enc, Ins_tk):
        ...
        # Projection head: [B, n_vars, d_model] -> [B, d_model, n_vars]
...
class EncoderLayer(nn.Module):
    def __init__(self, attention, d_model, d_ff=None, dropout=0.1, activation="relu",
                 num_experts=5, topk=2, router_modulation=False, modulation=True, lambda_e=0.75):
        ...
        # Expert-instruction Linear Modulation (EiLM)
        if self.modulation:
            print("EiLM enabled!")
    ...
    def forward(self, x, Ins_tk, attn_mask=None, tau=None, delta=None):
        ...
        tokens = x.reshape(-1, self.hidden_dim)  # [B*C, d]

        # Router with optional modulation
        if self.router_modulation and self.modulation and Ins_tk is not None:
            router_logits = self.Gate(tokens)  # [B*C, num_experts]
        ...
        if self.modulation == False:
            ...
            final_hidden_states = final_hidden_states.reshape(B, C, self.hidden_dim)
