...
from utils import get_transformer_backbone, normalize_timeseries
...
def main():
    ...
    model_dtype = dtype; embed_device = device
    ...
    if args.fusion_strategy.startswith('early_'):
        ts_encoder, align_layer, downstream_head = build_ts_and_align(
            args, embed_device, llm.config.hidden_size
        )
    ...
    print(f"LLM trainable parameter: {llm_trainable:.2f} M")
    ...
    for epoch in range(args.epochs):
        global_step, epoch_total_loss, epoch_batches = train_one_epoch(
            args, llm, ts_encoder, align_layer, downstream_head, train_loader, criterion,
