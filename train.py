...
def get_ts_embed(
    ...
    """
    Input: ts [B, in_len]
    Return inputs_embeds. The way to get inputs_embeds depends on the TS-Encoder you choose.  [B, n_var, in_len] for example
    """
    ...
    if args.ts_encoder == 'iTransformer':
        ...
        if len(ts.shape) == 2:
            ...
            ts = ts.permute(0, 2, 1) #[B, in_len, Channel]        #print(ts.shape)
...
def parse_args():
    ...
    parser.add_argument("--dataset_path", default="./data/processed/finance/long/train")
    ...
    parser.add_argument("--n_vars", type=int, default=1) # num of channals
...
def main():
    ...
    if args.ts_encoder == 'iTransformer':
        ...
        ts_encoder = MiTransformer(
            ...
            n_heads=2, n_layers=args.n_layers, modulation=args.modulation, lambda_e=args.lambda_e
            ).to(device)
        ...
        ts_encoder = MMLinear(in_len=args.in_len, out_len=args.out_len, top_k=args.topk, n_experts=args.n_experts, 
                              modulation=args.modulation, router_modulation=args.router_modulation, lambda_e=args.lambda_e).to(device)
        ...
        ts_encoder = MoMe(in_len=args.in_len, patch_len=args.patch_len, hidden_dim=args.hidden_dim, top_k=args.topk, num_experts=args.n_experts, 
                          modulation=args.modulation, router_modulation=args.router_modulation, lambda_e=args.lambda_e).to(device)
    ...
    elif args.ts_encoder in ['mmlinear', 'DLinear', 'TSMixer']:
        trainable_params = (
